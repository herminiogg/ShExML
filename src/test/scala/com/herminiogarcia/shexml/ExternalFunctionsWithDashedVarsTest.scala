...
class ExternalFunctionsWithDashedVarsTest extends AnyFunSuite with Matchers with RDFStatementCreator {
  ...
  private val example =
    """
      ...
      |SOURCE films-xml-file <https://shexml.herminiogarcia.com/files/films.xml>
      |SOURCE films-json_file <https://shexml.herminiogarcia.com/files/films.json>
      ...
      |    }
      |}
      |EXPRESSION films <films-xml-file.film_xml UNION films-json_file.film_json>
      |
      |:Films :[films.id] {
      |    :name [helper.allCapitals(films.na-me)] ;
      |    :year [helper.addOne(films.year-)] ;
      |    :countryOfOrigin dbr:[films.coun_tr-y-] ;
      |    :director dbr:[films.directors] ;
      |    :screenwritter dbr:[films.screenwritters] ;
      |    :screenwritterName [helper.getName(films.screenwritters)] ;
      |    :titleYear [helper.nameAndYear(films.na-me, films.year-)] ;
      |    :musicBy dbr:[films.music] ;
      |    :cinematographer dbr:[films.photography] ;
      |    :actor @:Ac-tor ;
      |    :actor @schema-org:Actress- ;
      |}
      |
      |:Ac-tor :[films.actors.name] {
      ...
      |}
      |
      |schema-org:Actress- :[films.actresses.name] {
