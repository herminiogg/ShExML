...
class UndefinedSourceInExpressionTest extends AnyFunSuite
  ...
  private val example =
    """
      ...
      |    }
      |}
      |EXPRESSION films <undeclared_file.film_xml UNION films_json_file.film_json>
      |
      |:Films :[films.id] {
      |    :name [films.name] ;
      |    :year [films.year] ;
  ...
  test("Undefined source in an expression is detected") {
