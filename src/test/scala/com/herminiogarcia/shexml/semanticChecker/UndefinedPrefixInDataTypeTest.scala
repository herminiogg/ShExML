...
class UndefinedPrefixInDataTypeTest extends AnyFunSuite
  ...
  private val example =
    """
      ...
      |:Films :[films.id] {
      |    :name [films.name] ;
      |    :year [films.year] wiki:[films.year] ;
  ...
  test("Undefined prefix in a datatype is detected") {
    ...
    assert(error.getEnrichedErrorMessage(example).contains("45:     :year [films.year] [1m[4mwiki:[22m[24m[films.year] ;"))
  }
