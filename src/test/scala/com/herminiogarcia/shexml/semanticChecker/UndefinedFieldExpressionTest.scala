...
class UndefinedFieldExpressionTest extends AnyFunSuite
  ...
  private val example =
    """
      ...
      |EXPRESSION goodExp <films_xml_file.film_xml.actors.name>
      |EXPRESSION erroredExp <films_xml_file.film_xml.country.name>
      |
      |:Films :[films.id] {
      |    :name [films.name] ;
      |    :year [films.year] ;
