...
class UndefinedVariableInDataTypeTest extends AnyFunSuite
  ...
  private val example =
    """
      ...
      |:Films :[films.id] {
      |    :name [films.name] ;
      |    :year [films.year] dbr:[films_xml_file.film_xml.nonexistent] ;
  ...
  test("Undefined variable in a datatype is detected") {
