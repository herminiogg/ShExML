...
class FilmsJoinLeftWithReviewsTest extends AnyFunSuite with Matchers with RDFStatementCreator {
  ...
  private val example =
    """
      ...
      |}
      |
      |EXPRESSION films <films_json_file.film_json JOIN films_with_reviews_file.film_with_reviews ON films_json_file.film_json.id=films_with_reviews_file.film_with_reviews.id>
      |
      |:Films :[films.name] {
      |    :name [films.name] ;
      |    :year :[films.year] ;
