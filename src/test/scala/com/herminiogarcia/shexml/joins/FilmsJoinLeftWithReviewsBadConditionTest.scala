...
class FilmsJoinLeftWithReviewsBadConditionTest extends AnyFunSuite
  ...
  private val example =
    """
      ...
      |}
      |
      |EXPRESSION films <films_json_file.film_json JOIN films_with_reviews_file.film_with_reviews ON films_json_file.film_json.name=films_with_reviews_file.film_with_reviews.id>
      |
      |:Films :[films.name] {
      |    :name [films.name] ;
      |    :year :[films.year] ;
