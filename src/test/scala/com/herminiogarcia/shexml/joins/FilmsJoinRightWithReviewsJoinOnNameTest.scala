...
class FilmsJoinRightWithReviewsJoinOnNameTest extends AnyFunSuite with Matchers with RDFStatementCreator {
  ...
  private val example =
    """
      ...
      |}
      |
      |EXPRESSION films <films_with_reviews_file.film_with_reviews JOIN films_json_file.film_json ON films_with_reviews_file.film_with_reviews.name=films_json_file.film_json.name>
      |
      |:Films :[films.id] {
      |    :year :[films.year] ;
