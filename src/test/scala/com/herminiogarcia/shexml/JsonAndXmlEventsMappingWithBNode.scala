...
/**
  ...
  */
class JsonAndXmlEventsMappingWithBNode extends AnyFunSuite with Matchers with RDFStatementCreator {
  ...
  test("Location shape is translated correctly") {
    ...
    assert(output.contains(createStatementWithLiteralAndBNodeSubject(output, prefix, "51.043613-3.717333", "long", "3.717333", XSDDatatype.XSDdecimal)))
    assert(output.contains(createStatementWithLiteralAndBNodeSubject(output, prefix, "51.043611-3.717222", "lat", "51.043611", XSDDatatype.XSDdecimal)))
    assert(output.contains(createStatementWithLiteralAndBNodeSubject(output, prefix, "51.043611-3.717222", "long", "3.717222", XSDDatatype.XSDdecimal)))
    assert(output.contains(createStatementWithLiteralAndBNodeSubject(output, prefix, "51.076891-3.717222", "lat", "51.076891", XSDDatatype.XSDdecimal)))
    assert(output.contains(createStatementWithLiteralAndBNodeSubject(output, prefix, "51.076891-3.717222", "long", "3.717222", XSDDatatype.XSDdecimal)))
  }
  ...
  test("Location shape is linked correctly in venue shape") {
    assert(output.contains(createStatementWithBNodeObject(output, prefix, "398", "location", "51.076891-3.717222")))
    assert(output.contains(createStatementWithBNodeObject(output, prefix, "567", "location", "51.043611-3.717222")))
    assert(output.contains(createStatementWithBNodeObject(output, prefix, "444", "location", "51.076891-3.717222")))
