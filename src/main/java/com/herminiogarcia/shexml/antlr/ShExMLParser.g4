...
functions: FUNCTIONS variable LESS_SYMBOL_QUERY (SCALA | JAVA) (URL | QUERY_PART) GREATER_SYMBOL_QUERY ;
...
action: prefixVar '[' expOrVarOrFunctionCallign (IF expOrVarOrFunctionCallign)? ']' ;
predicateObject: predicate (objectElement | shapeLink | literalValue) ;
objectElement: firstPartObjectElement (datatype | langtag)? ;
firstPartObjectElement: prefixVar? valueRetriever ;
valueRetriever: ('[' expOrVarOrFunctionCallign (MATCHING variable)? (IF expOrVarOrFunctionCallign)? (AS rdfCollection)? ']' | STRINGOPERATOR) ;
expOrVarOrFunctionCallign: (exp | variable | functionCalling) ;
...
datatype: XMLSCHEMADATATYPE | firstPartObjectElement | literalSubject ;
